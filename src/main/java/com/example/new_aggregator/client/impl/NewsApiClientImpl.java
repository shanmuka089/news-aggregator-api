...
public class NewsApiClientImpl implements NewsApiClient
{
    ...
    @Override
    public ResponseDto<NewsApiResponse> fetchNewsByPreference(QueryDto query)
    {
        String baseUrl = clientConfig.getNewsApiClient().getNewsApiBaseUrl();
        String newsApiSearchPath = clientConfig.getNewsApiClient().getNewsApiSearchPath();

        String url = ClientUtils.buildUrl(baseUrl, newsApiSearchPath, query, false);

        ResponseEntity<NewsApiResponse> responseEntity = restTemplate.exchange(
                url,
                HttpMethod.GET,
